...
def File2Dict(filename):
    ...
    while True:
        ...
        if(len(split_line)>=3):            # divide 1000
            alllines.append([float(split_line[0]), 
...
enc_eb1 = pl.errorbar(xtrue, enc_y_tls_draw, enc_ystd_tls, 
                      ecolor = 'magenta', linewidth=4.0, elinewidth = 2, marker = 'o', ms = 12)
enc_eb1[-1][0].set_linestyle('--')
enc_eb2 = pl.errorbar(xtrue, enc_y_unblock_draw, enc_ystd_unblock, 
                      ecolor = 'red', linewidth=4.0,elinewidth = 2, marker = '^', ms = 12)
enc_eb2[-1][0].set_linestyle('-.')
enc_eb3 = pl.errorbar(xtrue, enc_y_block_draw, enc_ystd_block, 
                      ecolor = 'black', linewidth=4.0, elinewidth = 2, marker = 'D', ms = 12)
enc_eb3[-1][0].set_linestyle(':')  

noenc_eb1 = pl.errorbar(xtrue, noenc_y_tls_draw, noenc_ystd_tls, 
                        ecolor = 'magenta', linewidth=4.0, elinewidth = 2, marker = 's', ms = 12)
noenc_eb1[-1][0].set_linestyle('--')
noenc_eb2 = pl.errorbar(xtrue, noenc_y_unblock_draw, noenc_ystd_unblock, 
                        ecolor = 'red', linewidth=4.0, elinewidth = 2, marker = '*', ms = 12)
noenc_eb2[-1][0].set_linestyle('-.')
noenc_eb3 = pl.errorbar(xtrue, noenc_y_block_draw, noenc_ystd_block, 
                        ecolor = 'black', linewidth=4.0, elinewidth = 2, marker = 'd', ms = 12)
noenc_eb3[-1][0].set_linestyle(':')  


#pl.title("END TO END DELAY ON DIFFERENT NUMBER OF CONNECTIONS")
pl.xlabel("Bandwidth (Mbps)", fontsize = 30)
...
pl.legend([enc_eb1, noenc_eb1, enc_eb2, noenc_eb2, enc_eb3, noenc_eb3],
          ["TLS", "TCP",
           ...
           "Non-Blocking (TCP)", 
           "Blocking (TLS)", 
           "Blocking (TCP)"], fontsize = 22,
          loc = 'upper right')
#ax.set_yscale('log', basex = 2)
pl.xticks(xtrue, xlabel)
pl.ylim(0, 30)
pl.xlim(0.5, 5.5)
#pl.ylim(0, max(max(enc_y_tls), max(enc_y_unblock), max(noenc_y_block), max(enc_y_block)) + 20)
#pl.grid(True)
pl.tight_layout()
pl.savefig('/Users/xizhaohan/Desktop/eva-1b.eps', format='eps', dpi=10000)    
pl.show()
